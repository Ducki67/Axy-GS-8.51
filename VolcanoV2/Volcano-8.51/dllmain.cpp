...
DWORD Main(LPVOID)
{
    ...
    // unlocked console!
    SetConsoleTitleA("Welcome to VolcanoV2 gs skidda!!");
    ...
    *(uint8_t*)Addr = 0x85;
    ...
    SetConsoleTitleA("VolcanoV2 (Process: Hooking...)");

    UptimeWebHook.send_embed("Server starting up..", "Wait untill the server is joinable!!");
    ...
    if (Globals::bNoMCP)
    {
        ...
        // maybe pickaxe
        MH_CreateHook((LPVOID)GetOffsetBRUH(0x16ddffc), NoMcp, nullptr);
        MH_EnableHook((LPVOID)GetOffsetBRUH(0x16ddffc));
    }
